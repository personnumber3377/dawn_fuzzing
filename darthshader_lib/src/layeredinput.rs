...
// These are helpers that need not a file.

impl LayeredInput {
    pub fn from_wgsl_bytes(bytes: &[u8]) -> Result<Self, ()> {
        let ast = crate::ast::Ast::try_from_wgsl(bytes)?;
        Ok(LayeredInput::Ast(ast))
    }

    pub fn to_wgsl_string(&self) -> Result<String, ()> {
        match self {
            LayeredInput::Ast(ast) => Ok(ast.get_text().clone()),
            LayeredInput::IR(ir) => ir.try_get_text().map(|s| s.clone()).map_err(|_| ()),
        }
    }
}
